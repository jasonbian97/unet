...
from utils import get_ids, split_ids, split_train_val, get_imgs_and_masks, batch, resize2, resize3, cvt2binary
...
def train_net(net,
    ...
    for epoch in range(epochs):
        ...
        for i, b in enumerate(batch(train, batch_size)):
            ...
            mask_list=[]
            for k in b:
                Xmean.append(k[0].shape[1])
                Ymean.append(k[0].shape[2])
            for k in b:
                # imgs_list.append(resize2(k[0],int(np.mean(Xmean)),int(np.mean(Ymean))))
                # mask_list.append(cvt2binary(resize2(k[1],int(np.mean(Xmean)),int(np.mean(Ymean)))))
                imgs_list.append(resize2(k[0],256,256))
                mask_list.append(cvt2binary(resize3(k[1],256,256)))
